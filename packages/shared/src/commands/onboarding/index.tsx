...
import { theme, BLOCK_FULL, Badge, Divider, ErrorBox, SelectList, Spinner } from '../../index.js';
import type { ColoredGlyph, TransitionSnapshot } from '../../transition.js';
import type { Translations } from '../../i18n/index.js';

export type Step = 'checking' | 'ready' | 'installing' | 'done' | 'error';
...
export interface OnboardingCommandProps {
  onBack: () => void;
  onComplete?: () => void;
  disableInput?: boolean;
  registerSnapshot?: (fn: () => TransitionSnapshot) => void;
  /** Check external dependencies - called with current config */
  checkDependencies: () => Promise<DependencyCheckResult>;
  /** Install a specific dependency by ID */
  installDependency: (id: string, onLog: (line: string) => void) => Promise<string>;
  /** Read current configuration */
  readConfig: () => { sevenZipPath?: string; picocryptPath?: string };
  /** Merge config updates */
  mergeConfig: (patch: { sevenZipPath?: string; picocryptPath?: string }) => void;
  /** Application name for display */
  appName?: string;
  /** Custom checkmark character */
  checkmark?: string;
  /** Custom pending/dot character */
  pendingChar?: string;
  /** Whether to auto-complete when installation is done. Default: true */
...
export function OnboardingCommand({
  ...
  useEffect(() => {
    ...
    registerSnapshot(() => ({
      rows: [
        ...
        ...(step === 'done' ? [cg(`${checkmark} ${t.depsInstalled}`, theme.success, true)] : []),
        ...(step === 'error' ? [cg(`${t.errorLabel} ${error}`, theme.warning)] : []),
        ...(status ? status.statuses.map((dep) =>
          cg(`${dep.installed ? checkmark : pendingChar} ${dep.label}`, dep.installed ? theme.success : theme.warning)
        ) : []),
      ],
    }));
  }, [registerSnapshot, step, status, error, checkmark, pendingChar, t]);
    ...
    void (async () => {
      try {
        ...
        for (const dep of missing) {
          ...
          const path = await installDependency(dep.id, pushLog);
          if (dep.id === 'sevenZip') {
  ...
  }, [step, status, checkDependencies, installDependency, mergeConfig, t]);
              ...
              {status.statuses.map((dep) => (
                <Box key={dep.id} gap={1}>
                  <Text
                    ...
                    bold
                  >
                    {dep.installed ? checkmark : pendingChar}
                  </Text>
        ...
        {step === 'done' && (
          <Box flexDirection="column" gap={1}>
            <Text color={theme.success} backgroundColor={theme.panelBg} bold>
              {checkmark} {t.onboardingComplete}
            </Text>
            <Text color={theme.dim} backgroundColor={theme.panelBg}>
              {t.preparingMenu}
            </Text>
